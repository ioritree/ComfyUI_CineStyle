...
async function openSelector(node, config) {
    ...
    if (anchorRow) { anchorRow.className = "cs-vseg-anchor-row"; anchorRow.innerHTML = `<span>Anchors</span><span class="cs-vseg-anchor-list"></span><button class="cs-vseg-button cs-vseg-delete-anchor" type="button">Delete Current Anchor</button><button class="cs-vseg-button cs-vseg-delete-all-anchors" type="button" title="Delete every anchor on the timeline">Delete All Anchors</button><button class="cs-vseg-button cs-vseg-prune-anchors" type="button" title="Delete every anchor of the previewed shot except the earliest one">Keep first anchor of this shot</button><span class="cs-vseg-anchor-warning"></span>`; dialog.querySelector(".cs-vseg-controls").after(anchorRow); }
    if (multiAnchor) {
        const toolGroup = shell?.querySelector(".cs-vseg-tool-group");
        const actions = dialog.querySelector(".cs-vseg-actions");
        const note = dialog.querySelector(".cs-vseg-fields");
        if (toolGroup) anchorRow.after(toolGroup);
        if (actions && toolGroup) toolGroup.after(actions);
    ...
    if (shotPanel) { shotPanel.className = "cs-vseg-shot-panel"; shotPanel.hidden = true; shotPanel.innerHTML = `<div class="cs-vseg-shot-row"><div class="cs-vseg-shot-head"></div><button class="cs-vseg-button cs-vseg-shot-toggle" type="button">Hide overlay</button><button class="cs-vseg-button cs-vseg-shot-adopt" type="button" title="Load the tracked mask of this frame into Draw Mask so it can be corrected">Adopt this frame</button><button class="cs-vseg-button cs-vseg-shot-clear" type="button">Clear preview</button></div><canvas class="cs-vseg-shot-curve" title="Mask area per frame. Red bands mark frames far below the typical area of this shot. Click to jump."></canvas><div class="cs-vseg-shot-thumbs"></div>`; shell.append(shotPanel); }
    // Result of the last Preview Current Shot run: per-frame overlays, areas and thumbnails.
    let shotPreview = null;
    let shotOverlayVisible = true;
    let frameEdited = false;
    let adoptToken = 0;
    let provisionalFrame = null;
    ...
    function updateAnchorMarkers() {
        ...
        // An anchor only tracks the objects prompted on it, so a half-filled
        // anchor silently drops the other object for its part of the shot.
        const missing = objects.map((object, index) => (objectPromptHasData(object) ? 0 : index + 1)).filter(Boolean);
        const warning = anchorRow.querySelector(".cs-vseg-anchor-warning");
        warning.textContent = frames.includes(frame) && missing.length && objects.length > 1
            ? `注意：Object ${missing.join("、")} 在這個錨點沒有提示，這一段不會追蹤它們。先按 Adopt this frame 載入追蹤結果，再補塗。`
            : "";
    }
    // Frames seeded from the tracked result are provisional: they are stored as
    // an anchor only after the user edits them.
    function stashAnchor() { if (!multiAnchor) return; const snapshot = promptSnapshot(); if (hasPromptData() && (frameEdited || anchorStore.has(frame))) anchorStore.set(frame, snapshot); else anchorStore.delete(frame); }
    async function adoptTrackedMasks({ silent = false } = {}) {
        if (silent) provisionalFrame = frame; else provisionalFrame = null;
        if (!config.shotMaskRoute || !shotPreview || frame < shotPreview.start || frame >= shotPreview.end) return false;
        const token = ++adoptToken;
        const result = await fetchPreview({ frame }, config.shotMaskRoute).catch((error) => { if (!silent) previewStatus.textContent = error.message; return null; });
        if (!result || token !== adoptToken || !result.objects?.length) return false;
        objectCount = Math.max(objectCount, ...result.objects.map((item) => Number(item.index) || 1));
        padObjects();
        await Promise.all(objects.map(ensureMaskCanvas));
        if (token !== adoptToken) return false;
        for (const item of result.objects) {
            const object = objects[Number(item.index) - 1];
            if (!object?.maskCanvas) continue;
            const image = new Image();
            await new Promise((resolve) => { image.onload = resolve; image.onerror = resolve; image.src = item.mask; });
            const context2d = object.maskCanvas.getContext("2d");
            context2d.clearRect(0, 0, object.maskCanvas.width, object.maskCanvas.height);
            context2d.drawImage(image, 0, 0, object.maskCanvas.width, object.maskCanvas.height);
            object.maskData = object.maskCanvas.toDataURL("image/png");
        }
        if (token !== adoptToken) return false;
        if (!silent) setOverlayVisible(false);
        updateObjectSelect(); updateAnchorPointer(); redraw();
        return true;
    }
    function padObjects() { if (!multiAnchor) return; while (objects.length < objectCount) objects.push(normalizePromptObject(null)); }
    async function loadAnchor() {
        const token = ++loadToken; const stored = anchorStore.get(frame); frameEdited = false; provisionalFrame = null; setOverlayVisible(true);
        objects = normalizePrompt(stored); padObjects(); activeObject = clamp(activeObject, 0, objects.length - 1);
        undoStack.length = 0; redoStack.length = 0; updateObjectSelect(); updateAnchorPointer(); redraw();
        await Promise.all(objects.map(ensureMaskCanvas));
        if (token !== loadToken) return;
        redraw();
        if (stored === undefined) await adoptTrackedMasks({ silent: true });
    }
    ...
    function setOverlayVisible(visible) {
        shotOverlayVisible = Boolean(visible);
        const toggle = shotPanel?.querySelector(".cs-vseg-shot-toggle");
        if (toggle) { toggle.textContent = shotOverlayVisible ? "Hide overlay" : "Show overlay"; toggle.classList.toggle("active", !shotOverlayVisible); }
    }
    function shotOverlay() { if (!shotOverlayVisible || !shotPreview || frame < shotPreview.start || frame >= shotPreview.end) return null; const image = shotPreview.overlays[frame - shotPreview.start]; return image?.complete ? image : null; }
    ...
    function commit(before) { const after = promptSnapshot(); if (before !== after) { undoStack.push(before); redoStack.length = 0; frameEdited = true; scheduleLivePreview(); if (shotPreview && !shotPreview.stale) { shotPreview.stale = true; renderShotHead(); } } syncAnchorFromPrompts(); }
    ...
    function brushCursor(erase = brushMode === "erase") {
        const diameter = clamp(Number(brushSize.value) || 32, 2, 100);
        const color = erase ? "#ff5b68" : "#35c98e";
    ...
    function updateCursor(position = null) { canvas.style.cursor = tool === "bbox" ? (bboxAddMode && !boxTargetAt(position || { x: -1, y: -1 }) ? "crosshair" : boxCursor(position ? boxTargetAt(position) : null)) : tool === "points" ? (position && pointIndexAt(position) >= 0 ? "grab" : "crosshair") : tool === "semantic" ? "text" : brushCursor(drag?.kind === "mask" ? drag.erase : undefined); }
    ...
    function paintMask(position, erase = brushMode === "erase") { const object = currentObject(); if (!object.maskCanvas) return; const maskContext = object.maskCanvas.getContext("2d"); const x = position.x * object.maskCanvas.width; const y = position.y * object.maskCanvas.height; const radius = Number(brushSize.value) * object.maskCanvas.width / Math.max(1, canvas.width) / 2; maskContext.save(); maskContext.globalCompositeOperation = erase ? "destination-out" : "source-over"; maskContext.fillStyle = "#fff"; maskContext.beginPath(); maskContext.arc(x, y, Math.max(1, radius), 0, Math.PI * 2); maskContext.fill(); maskContext.restore(); }
    ...
    canvas.addEventListener("pointerdown", (event) => {
        ...
        if (tool === "mask") { if (event.button !== 0 && event.button !== 2) return; event.preventDefault(); setOverlayVisible(false); const erase = event.button === 2 ? true : brushMode === "erase"; drag = { kind: "mask", erase, before: promptSnapshot(), pointerId: event.pointerId }; paintMask(position, erase); canvas.setPointerCapture?.(event.pointerId); hidePreview(); redraw(); return; }
    ...
    canvas.addEventListener("pointermove", (event) => { const position = eventPosition(event); if (!drag) { updateCursor(position); return; } if (drag.kind === "mask") { paintMask(position, drag.erase); hidePreview(); redraw(); return; } if (drag.kind === "point") { const rect = canvas.getBoundingClientRect(); if (!drag.moved && Math.hypot((position.x - drag.start.x) * rect.width, (position.y - drag.start.y) * rect.height) < 2) return; drag.moved = true; const point = currentObject().points[drag.pointIndex]; if (point) currentObject().points[drag.pointIndex] = { ...point, x: position.x, y: position.y }; canvas.style.cursor = "grabbing"; hidePreview(); redraw(); return; } updateBox(position); canvas.style.cursor = drag.target === "move" ? "grabbing" : boxCursor(drag.target); hidePreview(); redraw(); });
    ...
    dialog.querySelector(".cs-vseg-live").addEventListener("click", (event) => { livePreview = !livePreview; const button = event.currentTarget; button.textContent = `Live preview: ${livePreview ? "on" : "off"}`; button.classList.toggle("active", livePreview); if (livePreview) scheduleLivePreview(); else { window.clearTimeout(liveTimer); hidePreview(); } });
    ...
    // Switching to another video leaves every anchor of the old one behind,
    // and deleting them one frame at a time is slow once a clip has many shots.
    // Cancel still discards the whole edit, so this does not ask first.
    anchorRow?.querySelector(".cs-vseg-delete-all-anchors").addEventListener("click", () => {
        const frames = anchorFrames();
        if (!frames.length) { previewStatus.textContent = "There is no anchor to delete."; return; }
        anchorStore.clear();
        dialog.querySelector(".cs-vseg-clear").click();
        previewStatus.textContent = `已刪除 ${frames.length} 個錨點，按 Cancel 可放棄這次編輯。`;
    });
    // Stepping through a shot can leave an anchor on every visited frame; this
    // prunes a shot back to its first anchor in one go.
    anchorRow?.querySelector(".cs-vseg-prune-anchors").addEventListener("click", async () => {
        if (!shotPreview) { previewStatus.textContent = "Run Preview Current Shot first, so the shot range is known."; return; }
        const inside = anchorFrames().filter((value) => value >= shotPreview.start && value < shotPreview.end);
        if (inside.length <= 1) { previewStatus.textContent = "This shot already has a single anchor."; return; }
        const keep = inside[0];
        if (!window.confirm(`刪除鏡頭 ${shotPreview.start}–${shotPreview.end - 1} 內除了 ${keep} 以外的 ${inside.length - 1} 個錨點？`)) return;
        for (const value of inside.slice(1)) anchorStore.delete(value);
        if (frame !== keep && frame >= shotPreview.start && frame < shotPreview.end) { frameEdited = false; await loadAnchor(); }
        updateAnchorPointer(); redraw();
        previewStatus.textContent = `已刪除 ${inside.length - 1} 個錨點，保留 ${keep}。按 Preview Current Shot 重新追蹤。`;
    });
    // Turn the tracked mask of the current frame into prompts, so a missing arm
    // or leg can be painted in instead of redrawing the whole person.
    shotPanel?.querySelector(".cs-vseg-shot-adopt").addEventListener("click", async (event) => {
        const button = event.currentTarget; button.disabled = true; previewStatus.textContent = "Loading tracked mask...";
        try {
            const loaded = await adoptTrackedMasks();
            if (loaded) { setTool("mask"); previewStatus.textContent = `Frame ${frame} loaded from the tracked result. Paint the missing parts, then Preview Current Shot.`; }
        } finally { button.disabled = false; }
    });
    shotPanel?.querySelector(".cs-vseg-shot-toggle").addEventListener("click", () => { setOverlayVisible(!shotOverlayVisible); redraw(); });
    shotPanel?.querySelector(".cs-vseg-shot-clear").addEventListener("click", () => { shotPreview = null; provisionalFrame = null; setOverlayVisible(true); shotPanel.hidden = true; previewStatus.textContent = ""; redraw(); });
    ...
    shotButton?.addEventListener("click", async () => {
        ...
        try {
            ...
            frameEdited = anchorStore.has(frame); renderShotPanel(); void adoptTrackedMasks({ silent: true }); hidePreview(); previewStatus.textContent = `Shot ${result.start}–${result.end - 1} tracked. Scrub the timeline to inspect every frame.`; redraw();
    ...
    async function runFramePreview({ quiet = false } = {}) {
        if (previewButton.disabled) return;
        previewButton.disabled = true;
        previewStatus.textContent = quiet ? "Live preview..." : (config.previewLabel || "Running segmentation on this frame...");
        const token = ++previewToken;
        try {
            const snapshot = parseJson(promptSnapshot(), {});
            const prompted = (snapshot.objects || []).filter((object) => objectPromptHasData(normalizePromptObject(object)));
            if (!prompted.length) throw new Error("No object has a prompt on this frame yet.");
            const result = await config.preview({ node, filename, frame, previewFrame: sourceFrameForLocal(info, frame), info, promptData: JSON.stringify({ ...snapshot, objects: prompted }), fetchPreview: (payload) => fetchPreview({ ...payload, source_kind: source.kind || "video", source_token: source.token || "" }, config.previewRoute) });
            if (token !== previewToken) return;
            // The canvas stays visible: it is what receives clicks, so hiding it
            // would silently stop point/box editing while a preview is shown.
            previewImage.src = result.image; previewImage.style.display = "block"; setOverlayVisible(false);
            previewStatus.textContent = `Frame ${result.frame} · mask ${(Number(result.mask_area || 0) * 100).toFixed(1)}%${livePreview ? " · live" : ""}`;
        } catch (error) {
            canvas.style.visibility = "visible";
            if (!quiet || !/no object has a prompt/i.test(error.message)) previewStatus.textContent = error.message;
        } finally { previewButton.disabled = false; }
    }
    // Live mode re-runs the single-frame preview after each edit, so the next
    // point can be placed against the mask the previous one produced.
    function scheduleLivePreview() {
        if (!livePreview) return;
        window.clearTimeout(liveTimer);
        liveTimer = window.setTimeout(() => { if (!drag) void runFramePreview({ quiet: true }); }, 350);
    }
    previewButton.addEventListener("click", () => { semanticInput?.blur(); video.pause(); void runFramePreview(); });
